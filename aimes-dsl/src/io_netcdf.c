...
void io_read_init(GRID *g, const char* filename){
    int Error = nc_open(filename, NC_CLOBBER, & ncid_r); // | NC_NETCDF4
    PrintErrorAndExit(Error);
    InitIOBuffers(g);
}

// TODO: time this function
void io_read_register(GRID *g, const char * variable_name, GVAL * out_buff,
		      datatype_t file_type, datatype_t memory_type, GRID_VAR_POSITION var_pos,
		      GRID_VAR_DIMENSION var_dim)
{
    int ret;
    int varid;
	...
	else
	{
	    ...
	    GVAL EDGE 3D var = (GVAL *restrict*restrict*restrict) out_buff;
	    /*
	    ...
	    */
	    ...
	    {
		var[edge] = EdgeBuffer2D[edge.height][(edge.block*g->blkSize)+edge.edge];
	    }
	}
    }
    else
	...
	if(var_pos == GRID_POS_CELL)
	{
	    ...
	    */
	    ...
	    {
		var[cell] = CellBuffer1D[(cell.block*g->blkSize)+cell.cell];
	    }
	}
	else
	{
	    ...
	    */
	    ...
	    {
		var[edge] = EdgeBuffer1D[(edge.block*g->blkSize)+edge.edge];
	    }
	}
    }
    PrintErrorAndExit(Error);
...
void io_write_define(GRID *g, const char * variable_name, GVAL * out_buff,
		     datatype_t file_type,  GRID_VAR_POSITION var_pos,
...
void io_write_announce(GRID *g, io_var_t * var)
{
	...
	assert(!(WriteQueue.isLooped && (WriteQueue.Head==WriteQueue.Tail))); // NOTE: if this fires,
...
void io_write_start(GRID *g)
{
	...
	if(IoVar.Dim == GRID_DIM_3D)
	{
		...
		*/
		FOREACH cell IN grid
		{
		    CellBuffer2D[cell.height][(cell.block*g->blkSize)+cell.cell] = p[cell];
		}
		...
		//Error = nc_put_var_float(ncid_w, IoVar.id, &CellBuffer2D[0][0]);
	    }
	    else
	    {
		GVAL EDGE 3D p = IoVar.memory;
	...
	else
	{
		...
		*/
		...
		FOREACH cell IN gridCELL2D
		{
		    CellBuffer1D[(cell.block*g->blkSize)+cell.cell] = p[cell];
		}
		...
		*/
		...
		FOREACH edge IN gridEDGE2D
		{
		    EdgeBuffer1D[(edge.block*g->blkSize)+edge.edge] = p[edge];
		}
