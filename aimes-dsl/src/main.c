...
static option_help options [] = {
  {'T', "timesteps", "The number of timesteps.", OPTION_OPTIONAL_ARGUMENT, 'd', & timesteps},
  ...
  {'M', "maxtime", "The maximum runtime in seconds.", OPTION_OPTIONAL_ARGUMENT, 'F', & maxtime},
  {'m', "mintime", "The minimum time in seconds.", OPTION_OPTIONAL_ARGUMENT, 'F', & mintime},
  {'R', "io_init_file", "The I/O initialization file (to read initial values).", OPTION_REQUIRED_ARGUMENT, 's', &io_init_file},
  {'W', "io_output_file", "The output file.", OPTION_REQUIRED_ARGUMENT, 's', &io_output_file},
  {'t', "test_netcdf_io", "Tests the writing and reading of netcdf files with random values.", OPTION_OPTIONAL_ARGUMENT, 'd', &test_netcdf_io},
  LAST_OPTION
};
...
int main(int argc, char** argv)
{
    ...
    INITCOMMLIB;
    init_grid(g,gridsize,gridheight);//currently grid size for one dimension
    ...
    double dt = 0;
    double st = mtime();
    timestep(ts=0; ((ts < timesteps) || (dt < mintime)) && dt < maxtime; ts++){
        /*com1.compute(g);
        com2.compute(g);
        com3.compute(g);
        com4.compute(g);*/
    ...
    {
        double gflops,mem;
        gflops = (/*com1.flops(g) + com2.flops(g) + com3.flops(g) + com4.flops(g) +*/ com5.flops(g))*ts/(1024*1024*1024)/dt;
        mem = (com1.memory(g)/* + com2.memory(g) + com3.memory(g) + com4.memory(g) + com5.memory(g)*/);
        printf("%ld,%f,%f,%f\n",ts,dt,gflops,mem);
